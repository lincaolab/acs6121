...
1. The node is called `colour_search.py`, and it is located in the `src` folder of the `tuos_examples` package. Copy this into the `src` folder of your own `week6_vision` package by first ensuring that you are located in the desired destination folder:
...
1. In the same way as last week, you'll also need to copy the `tb3.py` module across from the `tuos_examples` package too, as this is used by the `colour_search.py` node to make the robot move:
